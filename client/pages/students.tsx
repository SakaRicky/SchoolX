...
const StudentsPage = () => {
	...
	const [currentClassList, setCurrentClassList] = useState<Student[] | null>(
		null
	);
	...
	const fetchAllClasses = async () => {
		try {
			...
			setAllClassLists(allClasslist.sort());
		} catch (error: any) {
			console.log(error);
		}
	};
	...
	const handleChange = (event: ChangeEvent<HTMLInputElement>) => {
		...
		const selectedClass = allClassLists.filter(
			c => c.classCode === event.target.value
		);

		setCurrentClassList(selectedClass[0].students);
	};

	// filter according to search state or return the whole data
	const filteredStudents = searchText
		? currentClassList?.filter(data => {
	...
	const handleSelected = (selectedStudents: StudentOnClassList[]) => {
		...
		selectedStudents?.length === 1
			? setEditStudent(selectedStudents[0])
	...
	return (
		<div className={styles.root}>
			<Modal open={openEditStudentModal} onClose={handleCloseModal}>
			...
			<Modal open={openAddStudentModal} onClose={handleCloseModal}>
				{/* 
                ...
                */}
				<>
					<AddStudentForm
						...
						handleSubmit={handleSaveNewStudent}
						ref={ref}
					/>
				</>
			</Modal>
				...
				<Grid item xs={12} sm={6} className={styles.selectTextField}>
					...
					>
						{allClassLists.map(classList => {
							return (
								<MenuItem key={classList.classCode} value={classList.classCode}>
									{classList.className}
								</MenuItem>
							);
						})}
					...
					<p>
						<small>* Select a class to view students</small>
					</p>
				</Grid>
				<Grid item className={styles.button}>
					{editStudent ? (
						<Button handleClick={handleEdit} label="Edit Student">
							<EditIcon />
						</Button>
					) : (
						<Button handleClick={handleAddStudent} label="Add Student">
							<PersonAddIcon />
						</Button>
					)}
				</Grid>
			...
			<div className={styles.table}>
				...
				{filteredStudents && (
					<ClassListTable
						classList={filteredStudents}
						setSelectedStudents={handleSelected}
					/>
				)}
			</div>
		</div>
